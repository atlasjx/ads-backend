...
def apply_schema(conn):
    ...
    for stmt in [s.strip() for s in sql_schema.split(';') if s.strip()]:
        cur.execute(stmt)
    # Align numeric columns with dataset ranges
    cur.execute("ALTER TABLE movies ALTER COLUMN id SET GENERATED BY DEFAULT;")
    cur.execute("""
        ALTER TABLE movies
            ALTER COLUMN budget TYPE BIGINT USING budget::bigint,
            ALTER COLUMN revenue TYPE BIGINT USING revenue::bigint,
            ALTER COLUMN vote_count TYPE BIGINT USING vote_count::bigint;
    """)
    conn.commit()
    cur.close()
    ...
    with open(csv_path, newline='', encoding='utf-8') as csvfile:
        ...
        for row_num, row in enumerate(reader, start=1):
            try:
                ...
                if movie_id is None:
                    logging.warning(f"Row {row_num}: Skipping movie with invalid id value: {row.get('id')}")
                    continue
                ...
                """, (
                    movie_id, row.get('imdb_id'), row.get('title'), row.get('original_title'),
